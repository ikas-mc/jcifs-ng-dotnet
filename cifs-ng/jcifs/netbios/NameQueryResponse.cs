...
namespace jcifs.netbios {
	...
	internal class NameQueryResponse : NameServicePacket {
		...
		internal NameQueryResponse(Configuration config) : base(config) {
		...
		internal override int readRDataWireFormat(byte[] src, int srcIndex) {
			...
			bool groupName = ((src[srcIndex] & 0x80) == 0x80) ? true : false;
		...
		public override string ToString() {
			return "NameQueryResponse[" + base.ToString() + ",addrEntry=" + this.addrEntry?.joinToString() + "]";
		}
	}
